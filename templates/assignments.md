...
**Author attribution:** Set `created_by` (assigned_by identity) and seed `contributors` on creation. Append current git identity to `contributors` on status change and on completion (dedup by email). Full rules: `conventions/maintainer.md` → Author Attribution. Schema: `conventions/maintainer.md` → Author Attribution.
