...
| 4 | **Open-phase commit gap** | Same as full — find open phases (`status` in `planning|implementation|review`); compute branch commits not in `phase.yml.commits`; same-user auto-assign by file overlap; escalate on author mismatch / ambiguous. | `Read: phases/index.yml`; `Bash: git log ...` | **Escalate** | **high** |
