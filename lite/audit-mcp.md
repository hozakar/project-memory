...
3. Receive `{ auto_fixed, pending_fixes, escalations }`:
   ...
   - `pending_fixes`: Cat 7 orphan annotations — apply each using the Edit tool (annotate the hash in `phases/index.yml` and the corresponding `phases/<phase_id>/phase.yml`).
