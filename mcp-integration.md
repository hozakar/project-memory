...
| `run_audit(project_memory_dir)` | Executes all 5 audit categories in a single MCP call. Returns `{auto_fixed, pending_fixes, escalations}`. See `audit-mcp.md`. |
| `apply_audit_fixes(project_memory_dir, pending_fixes)` | Deterministically applies the `pending_fixes` payload from `run_audit`. Supports 5 fix types: `assign_commit`, `add_decision_index_row`, `fix_decision_index_status`, `assign_adr_id`, `create_adr_file`. Returns `{applied, partial, failed, rerun_audit_recommended}`. Source-of-truth safe (never reads vector index, never synthesizes prose). Idempotent. See `audit-mcp.md` step 2. |
