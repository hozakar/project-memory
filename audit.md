...
- **Cat 4 — branch:null date filtering:** When `branch: null` and a fallback branch (main/master/staging) is used, filter commits via `--after=<started_at>` to exclude pre-project historical commits. This prevents false positives for long-lived branches where historical commits appear as gaps for a newly-opened phase. If `started_at` is absent, fall back to unfiltered behavior.
