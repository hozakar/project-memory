...
2. Call `run_audit(project_memory_dir, { profile: "standard" })` (background omitted/false) for explicit `Skill project-memory audit` invocation. The MCP server will:
   - Internally use the active category set defined in standard/audit-fs.md. Cat 9 and Cat 11 (discussion hygiene) remain active for all profiles (legacy profiles normalize to standard at read time).
