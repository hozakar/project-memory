...
3. We migrated our API gateway roughly two years ago. What decisions cluster around that
...
6. Which records (decisions or discussions) involved migration work — moving data, schema changes,
   or traffic cut-overs — that could have introduced silent data-integrity risks?
   What mitigations were recorded?
...
8. Across all discussions tagged with "infrastructure" or "deployment", what recurring failure modes or
...
10. Find all discussions that concluded with outcome type "none" (no decision). Do any of
...
12. We are considering moving from REST to gRPC for all internal service communication. List every
    decision that assumes HTTP/REST semantics, ordered by how hard they would be to reverse.
...
13. Our new CISO wants to switch password hashing from bcrypt to Argon2id. Does any existing
    ...
    there any downstream decisions that reference the hashing algorithm directly?
...
15. The analytics team proposes moving the data warehouse from the current solution to a columnar
    lakehouse. Which decisions defined ETL pipelines, data models, or data-retention policies that
    assume the current warehouse schema or API, and which would need to change alongside the migration?
