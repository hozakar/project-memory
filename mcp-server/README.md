...
- `run_audit(project_memory_dir, profile?, raise_cat4?)` — execute all 14 audit categories; returns {auto_fixed, pending_fixes, escalations} with pre-computed interactive flags; accepts `profile` parameter (`full` | `lite` | `minimal`), `raise_cat4` boolean for open-phase gap escalation
...
- `find_touching_phases(file_path)` — find which phases touched a given file via git log + commit hash matching
