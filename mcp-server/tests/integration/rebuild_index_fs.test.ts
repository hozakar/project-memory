...
function cleanSubdirs(pmDir: string): void {
  const subdirs = ["decisions", "discussions", "instructions", "notes"];
...
describe("rebuildIndex with mode:fs", () => {
  it(
    ...
    async () => {
      ...
      const results = await searchMemory({ query: "filesystem rebuild scanning decisions", topK: 5 });
  ...
  it(
    ...
    async () => {
      ...
      const results = await searchMemory({ query: "rebuildIndex mode fs mixed type", topK: 10 });
  ...
  it(
    ...
    async () => {
      ...
      const results = await searchMemory({ query: "skip test valid decision", topK: 5 });
      expect(
  ...
  it(
    ...
    async () => {
      ...
      const noteResults = await searchMemory({ query: "FS rebuild note test", topK: 10, includeCommits: false, createdByEmail: "note-tester@test.com", typeFilter: "note" });
      ...
      const broadResults = await searchMemory({ query: "FS rebuild note test", topK: 10 });
      expect(
  ...
  // removed: "indexes assignment files and makes them searchable with assignedToEmail"
  // — assignment feature dropped 2026-07-29
  ...
  it(
    ...
    async () => {
      ...
      // Verify via search
      const results = await searchMemory({ query: "backward compat entries API decision", topK: 5 });
  ...
  it(
    ...
    async () => {
      ...
      // Verify via search
      const results = await searchMemory({ query: "colon in value breaks YAML", topK: 3 });
