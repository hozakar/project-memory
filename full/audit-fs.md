...
**Note on `applies_globally` field and `Global` column:** The `applies_globally` field on DECISION frontmatter and the `Global` column in `decisions/index.md` (added per `DECISION-2026-06-17-global-scope-decisions`) are non-breaking. Cat 6 currently validates by `id` and `status` only — the new field and column are invisible to drift detection and are accepted as legitimate metadata. Drift detection for `applies_globally ↔ Global` consistency is deferred to a future audit category.
...
| 4 | **Open-phase commit gap** | In `phases/index.yml`, find every phase with `status` equal to `planning`, `implementation`, or `review`. For each: identify branch (if null, try `main`, `master`, `staging`; if none exist, skip). Get commits already in `commits[]`. Run `git log --oneline --max-count=200 --after=<started_at - 1 day> <branch>` and find commits on branch not in the phase's list. Same-user heuristic: auto-assigns commits to current or next phase by file overlap scoring. Unresolved (author mismatch or ambiguous) → **info on on-load**, escalate to interactive triage on manual audit. Age: timestamp of the earliest untracked commit. | `Read: .project-memory/phases/index.yml`; `Bash: git log --oneline --max-count=200 --after=<started_at - 1 day> <branch>` | **Info (on-load) / Escalate (manual audit)** | **high** |
